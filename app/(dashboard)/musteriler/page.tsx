...
export default function MusterilerPage() {
  ...
  const SEG_COLOR: Record<string, {bg:string,color:string,border:string}> = {
    ...
    aktif: { bg: 'rgba(58,55,48,0.4)', color: '#8A8580', border: 'rgba(58,55,48,0.3)' },
    yeni: { bg: 'rgba(31,29,23,0.6)', color: '#6B6760', border: 'rgba(58,55,48,0.2)' },
  ...
  const card = { background: '#111009', border: '1px solid rgba(201,168,76,0.1)', borderRadius: 12 }
  const th = { padding: '12px 18px', textAlign: 'left' as const, fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase' as const, color: '#3A3730', fontWeight: 500, background: 'rgba(201,168,76,0.02)' }
  ...
  return (
    <div style={{ padding: '32px 28px', maxWidth: 1280, margin: '0 auto' }}>
      <header style={{ marginBottom: 28 }}>
        <p style={{ fontSize: 10, letterSpacing: '0.3em', textTransform: 'uppercase', color: '#3A3730', fontFamily: 'JetBrains Mono, monospace', marginBottom: 6 }}>{customers.length} müşteri</p>
        <h1 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 44, fontWeight: 300, color: '#F5F0E8', letterSpacing: '-0.03em', lineHeight: 1 }}>Müşteriler</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 20 }}>
        {([['vip','VIP','3+ sipariş veya abone'],['aktif','Aktif','Son 30 günde'],['yeni','Yeni','30-90 gün'],['kayip','Kayıp','90+ gün önce']] as const).map(([key,label,sub]) => (
          <div key={key} onClick={() => setFilter(filter === key ? 'all' : key)}
            style={{ ...card, padding: '18px 20px', cursor: 'pointer', borderColor: filter === key ? 'rgba(201,168,76,0.3)' : 'rgba(201,168,76,0.1)', background: filter === key ? 'rgba(201,168,76,0.06)' : '#111009' }}>
            <p style={{ fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: SEG_COLOR[key].color, marginBottom: 10 }}>{label}</p>
            <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 40, fontWeight: 300, color: '#F5F0E8', lineHeight: 1 }}>{segmentStats[key]}</div>
            <p style={{ fontSize: 10, color: '#3A3730', fontFamily: 'JetBrains Mono, monospace', marginTop: 8 }}>{sub}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
        <div style={{ position: 'relative', flex: 1 }}>
          ...
          {search && <button onClick={() => setSearch('')} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: '#3A3730', cursor: 'pointer' }}><X size={14} /></button>}
        </div>
        {filter !== 'all' && <button onClick={() => setFilter('all')} style={{ padding: '0 16px', background: '#111009', border: '1px solid rgba(201,168,76,0.1)', borderRadius: 8, fontSize: 12, color: '#6B6760', cursor: 'pointer' }}>Filtreyi Kaldır</button>}
      </div>

      {/* Tablo */}
      <div style={{ ...card, overflow: 'hidden' }}>
        {loading ? <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 8 }}>{[1,2,3,4,5].map(i => <div key={i} style={{ height: 52, background: '#181612', borderRadius: 8 }} />)}</div> : (
                ...
                <tr key={c.phone} onClick={() => setSelected(c)} style={{ cursor: 'pointer' }}
                  ...
                  <td style={td}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <span style={{ color: '#C8C0B0', fontSize: 12 }}>{c.name || '—'}</span>
                    </div>
                  </td>
                  <td style={{ ...td, fontFamily: 'JetBrains Mono, monospace', color: '#6B6760' }}>{c.phone}</td>
                  ...
                  <td style={{ ...td, color: '#3A3730', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.session?.musteri_yazdigi || '—'}</td>
                  <td style={td}><span style={{ fontSize: 11, color: c.session?.kvkk_onay ? '#C9A84C' : '#3A3730' }}>{c.session?.kvkk_onay ? '✓' : '✗'}</span></td>
                  <td style={{ ...td, fontFamily: 'JetBrains Mono, monospace', color: '#3A3730', whiteSpace: 'nowrap' }}>{c.session ? formatDistanceToNow(new Date(c.session.updated_at), { addSuffix: true, locale: tr }) : '—'}</td>
                  <td style={{ ...td, color: '#3A3730' }}>›</td>
                </tr>
              ))}
            </tbody>
          </table>
        )}
        {!loading && filtered.length === 0 && <div style={{ padding: 40, textAlign: 'center', fontSize: 11, color: '#3A3730', fontFamily: 'JetBrains Mono, monospace' }}>müşteri bulunamadı</div>}
      </div>
          ...
          <div style={{ position: 'relative', width: '100%', maxWidth: 420, background: '#0D0B09', height: '100%', overflowY: 'auto', borderLeft: '1px solid rgba(201,168,76,0.15)' }} onClick={e => e.stopPropagation()}>
            <div style={{ position: 'sticky', top: 0, zIndex: 10, background: 'rgba(13,11,9,0.95)', backdropFilter: 'blur(12px)', padding: '20px 24px', borderBottom: '1px solid rgba(201,168,76,0.08)', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <div style={{ fontSize: 14, fontWeight: 500, color: '#F5F0E8' }}>{selected.name || '—'}</div>
                  <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: '#6B6760', marginTop: 2 }}>{selected.phone}</div>
              ...
              <button onClick={() => setSelected(null)} style={{ width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#181612', border: '1px solid rgba(201,168,76,0.1)', borderRadius: 8, color: '#6B6760', cursor: 'pointer', flexShrink: 0 }}><X size={14} /></button>
            </div>
            ...
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
                {[{label:'Sipariş',value:selected.orderCount||'—'},{label:'Harcama',value:selected.totalSpent>0?`${selected.totalSpent.toLocaleString('tr')} ₺`:'—'},{label:'Haftalık',value:selected.abonelik?.haftalik_adet||'—'}].map(({label,value}) => (
                  <div key={label} style={{ background: '#181612', border: '1px solid rgba(201,168,76,0.08)', borderRadius: 10, padding: '14px 12px', textAlign: 'center' }}>
                    <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 28, fontWeight: 300, color: '#F5F0E8', lineHeight: 1 }}>{value}</div>
                    <div style={{ fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#3A3730', marginTop: 6 }}>{label}</div>
                  </div>
                ))}
              </div>
              ...
              {selected.session && (
                <div style={{ background: '#181612', border: '1px solid rgba(201,168,76,0.08)', borderRadius: 10, padding: '14px 16px' }}>
                  <p style={{ fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#3A3730', marginBottom: 12 }}>WhatsApp</p>
                  {[
                    ...
                    <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid rgba(201,168,76,0.04)' }}>
                      <span style={{ fontSize: 11, color: '#3A3730' }}>{label}</span>
                      <span style={{ fontSize: 11, color: '#8A8580', maxWidth: 180, textAlign: 'right', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{value}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <a href={`https://wa.me/${selected.phone}`} target="_blank" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '11px 0', background: '#181612', border: '1px solid rgba(201,168,76,0.1)', borderRadius: 8, color: '#6B6760', fontSize: 13, textDecoration: 'none' }}>WhatsApp'ta Aç →</a>
              </div>
            </div>
          </div>
        </div>
