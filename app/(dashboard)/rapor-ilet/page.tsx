...
export default function RaporIletPage() {
  ...
  const inp = (label: string, key: keyof typeof form, placeholder: string, prefix?: string) => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', background: 'var(--bg-card)', border: '1px solid rgba(201,168,76,0.12)', borderRadius: 8, overflow: 'hidden' }}>
        ...
        <input type={prefix ? "number" : "text"} value={form[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))} placeholder={placeholder}
          style={{ flex: 1, background: 'none', border: 'none', outline: 'none', padding: '12px 14px', fontSize: 13, color: 'var(--text-primary)', fontFamily: 'inherit' }} />
      </div>
    </div>
  )
      ...
      <div className="rapor-form" style={{ flex: '0 0 480px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ background: 'rgba(201,168,76,0.05)', border: '1px solid rgba(201,168,76,0.2)', borderRadius: 12, padding: '16px 20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--gold-light)' }}>
          ...
          {loadingAylik ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div>
                ...
                <p style={{ margin: 0, fontFamily: 'Cormorant Garamond, serif', fontSize: 30, fontWeight: 300, color: 'var(--gold-light)' }}>{ayKumulatifAdet} <span style={{ fontSize: 12, color: '#7A7468' }}>adet</span></p>
              </div>
              <div>
                <p style={{ margin: '0 0 2px', fontSize: 9, color: '#7A7468', letterSpacing: '0.15em', textTransform: 'uppercase' }}>Toplam Ciro</p>
                <p style={{ margin: 0, fontFamily: 'Cormorant Garamond, serif', fontSize: 30, fontWeight: 300, color: 'var(--gold-light)' }}>{fmt(ayKumulatifTutar)} <span style={{ fontSize: 12, color: '#7A7468' }}>₺</span></p>
        ...
        {/* Trendyol */}
        <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(201,168,76,0.1)', borderRadius: 12, padding: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
            <TrendingUp size={14} color="#C9A84C" strokeWidth={1.5} />
            <span style={{ fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--gold-light)' }}>Trendyol</span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div style={{ background: 'rgba(201,168,76,0.04)', border: '1px solid rgba(201,168,76,0.15)', borderRadius: 12, padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 24 }}>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 26, color: 'var(--gold-light)', fontWeight: 300 }}>{total_adet}</div>
                <div style={{ fontSize: 9, color: '#5A5550', textTransform: 'uppercase', letterSpacing: '0.1em' }}>adet</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 26, color: 'var(--gold-light)', fontWeight: 300 }}>{fmt(total_tutar)} ₺</div>
          ...
          <div style={{ background: '#F8F5F0', borderRadius: 12, overflow: 'hidden', border: '1px solid rgba(139,105,20,0.15)' }}>
            <div style={{ background: '#EDE8E0', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 8, borderBottom: '1px solid rgba(139,105,20,0.12)' }}>
              <div style={{ display: 'flex', gap: 5 }}>
                {['#C4364A','var(--gold-light)','#7AC98A'].map(c => <div key={c} style={{ width: 10, height: 10, borderRadius: '50%', background: c, opacity: 0.6 }} />)}
              </div>
            ...
            <div style={{ padding: 0, fontFamily: 'Georgia, serif' }}>
              ...
              <div style={{ padding: '24px 40px', background: '#1A1610', borderBottom: '1px solid rgba(201,168,76,0.1)' }}>
                <p style={{ margin: '0 0 14px', fontSize: 8, letterSpacing: '3px', textTransform: 'uppercase', color: 'var(--gold-light)' }}>{ayLabel} — Aylık Toplam (Bugün Dahil)</p>
                <div style={{ display: 'flex', gap: 32 }}>
                  <div>
                    <p style={{ margin: '0 0 2px', fontSize: 9, color: '#7A7570' }}>Toplam Satış</p>
                    <p style={{ margin: 0, fontSize: 36, fontWeight: 300, color: 'var(--gold-light)', lineHeight: 1 }}>{ayKumulatifAdet} <span style={{ fontSize: 12, color: '#7A7570' }}>adet</span></p>
                  </div>
                  <div>
                    <p style={{ margin: '0 0 2px', fontSize: 9, color: '#7A7570' }}>Toplam Ciro</p>
                    <p style={{ margin: 0, fontSize: 36, fontWeight: 300, color: 'var(--gold-light)', lineHeight: 1 }}>{fmt(ayKumulatifTutar)} <span style={{ fontSize: 12, color: '#7A7570' }}>₺</span></p>
              ...
              <div style={{ padding: '24px 40px', background: '#FFFFFF', borderBottom: '1px solid rgba(139,105,20,0.1)' }}>
                <p style={{ margin: '0 0 12px', fontSize: 8, letterSpacing: '3px', textTransform: 'uppercase', color: 'var(--gold-light)' }}>Trendyol</p>
                <div style={{ display: 'flex', gap: 32 }}>
